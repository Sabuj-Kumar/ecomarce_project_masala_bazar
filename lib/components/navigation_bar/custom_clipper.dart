...
class Clipper extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.addRRect(RRect.fromLTRBR(size.width * 0.02, size.height * 0.4,
        size.width * 0.98, size.height, const Radius.circular(15)));
    path.moveTo(size.width * 0.38, size.height * 0.4);
    path.quadraticBezierTo(size.width * 0.41, size.height * 0.02,
        size.width * 0.5, size.height * 0.01);
    path.quadraticBezierTo(size.width * 0.59, size.height * 0.02,
...
class CustomClipperDown extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    path.quadraticBezierTo(x * 0.41, y * 0.077, x * 0.49,
        y * 0.068); // control point 1 curve left middle
    path.quadraticBezierTo(x * 0.57, y * 0.105, x * 0.6,
        y * 0.35); //control point 3 curve right middle
...
class CustomClipperUp extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    path.quadraticBezierTo(x * 0.41, y * 0.076, x * 0.48,
        y * 0.045); // control point 1 curve left middle
    path.quadraticBezierTo(x * 0.565, y * 0.07, x * 0.6,
        y * 0.35); //control point 3 curve right middle
    path.quadraticBezierTo(x * 0.6, y * 0.39, x * 0.65, y * 0.4);
    path.lineTo(x * 0.93, y * 0.4); //cornerCurveStarting point
    path.quadraticBezierTo(x, y * 0.4, x, y * 0.6);
